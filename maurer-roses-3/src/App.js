...
export default class App extends BaseApp {
  ...
  createAudioActivationButton() {
    ...
    button.addEventListener("click", () => {
      this.audioCtx.resume().then(() => {
        ...
        button.remove(); // Supprime le bouton après activation
      });
    });
  ...
  initAudio() {
    ...
    const audioUrl = "/sounds/frequency2.mp3";
    fetch(audioUrl)
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Erreur HTTP ${response.status} lors du chargement de ${audioUrl}`);
        }
  ...
  checkIntersectionProximity(playerPoint) {
    const threshold = 10;
    const nearbyPoints = this.intersections.filter(
      (point) =>
        Math.abs(point.x - playerPoint.x) < threshold &&
        Math.abs(point.y - playerPoint.y) < threshold
    );
    ...
    for (const point of nearbyPoints) {
      ...
      if (dist < threshold) {
        if (!this.lastTrigger || Date.now() - this.lastTrigger > 50) {
